 ...
 *
 * A freshly flashed/uncommissioned Node has zero Loads/Branches until a
 * COMMISSION_COMMAND from Central assigns it a friendly name (see
 * lib/NodeIdentityStore, lib/CommissioningPackets) — handled inline inside
 * the ESP-NOW Communication Task's own receive dispatch, the same place
 * RELAY_COMMAND already is.
 *
 ...
 * read or write Load state concurrently. identityMutex separately protects
 * identityStore, written by the ESP-NOW Communication Task and read by both
 * it and the Watchdog Task.
 */
...
/** How long to wait for Central's NODE_REPORT_ACK before logging CENTRAL_ACK_TIMEOUT. */
constexpr std::uint32_t NODE_REPORT_ACK_TIMEOUT_MS = 3000U;

/*
 * Upstream discovery is retried here, asynchronously, from inside
 * espNowCommunicationTask's own loop — never as a blocking loop in
 * app_main() before local tasks even start. UPSTREAM_DISCOVERY_ATTEMPT_TIMEOUT_MS
 * is deliberately short so one retry never meaningfully delays
 * RELAY_COMMAND routing or NODE_REPORT sending; UPSTREAM_DISCOVERY_RETRY_PERIOD_MS
 * paces how often a retry is attempted while no route exists.
 */
...
/**
 * Applies a queued RelayCommandPacket through RelayController, reads back
 * the confirmed state, updates the owning Load's physical truth, and
 * replies with a RelayCommandAcknowledgementPacket — never presenting the
 * requested state as confirmed until RelayController itself confirms it.
 */
        ...
        if (xSemaphoreTake(nodeMutex, pdMS_TO_TICKS(200U)) == pdTRUE) {
            ...
            if (load != nullptr) {
                if (acknowledgement.success != 0U) {
                     ...
                     * OFF — leave the previous trusted confirmed
...
/**
 * Establishes a safe physical relay state, erases every
 * installation-specific persisted record (a full NVS erase — the ESP32's
 * MAC address and chip identity live in efuses, never NVS, so this can
 * never touch immutable hardware identity), and reboots into PRODUCTION +
 * UNCOMMISSIONED. Never returns.
 *
 * This is a deliberate, explicit action distinct from uploading new
 * firmware — a normal firmware upload does not call this and does not
 * erase NVS.
 */
...
/**
 * Periodically sends this Node's report toward Central, and otherwise
 * reproduces the dynamic tree behaviour already proven for this project —
 * learning NODE_REPORTs, forwarding traffic up/down the tree, routing an
 * addressed RELAY_COMMAND to the Relay Control Task's queue, and applying
 * an addressed COMMISSION_COMMAND/DECOMMISSION_COMMAND.
 */
void espNowCommunicationTask(void *parameter)
{
    ...
    /*
     * Tracks the one outstanding NODE_REPORT this task is waiting on
     * Central to acknowledge. Local to this task (only it ever sends a
     * NODE_REPORT or receives a NODE_REPORT_ACK), so no mutex is needed
     * for these.
     */
...
/**
 * Reports link/relay health — this Node holds its last commanded relay
 * state while a route to Central is unavailable. Discovery retry itself is
 * owned entirely by espNowCommunicationTask; this task only reports the

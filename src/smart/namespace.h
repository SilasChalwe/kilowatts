    ...
    std::uint16_t sequence)
{
    ...
    for (std::size_t i = 0U; i < packet.numberOfLoads; ++i) {
        ...
        out.startupWatts = load->getPower().startupWatts;
    ...
    while (true) {
        ...
        acknowledgement.failureReason = static_cast<std::uint8_t>(
            success ? RelayCommandFailureReason::NONE
                    : (!writeOk ? RelayCommandFailureReason::GPIO_WRITE_FAILED
                                : (!readOk ? RelayCommandFailureReason::RELAY_PIN_NOT_REGISTERED

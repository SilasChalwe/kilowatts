/**
 * @file test_node_load_hardware_store.cpp
 * @brief Host-native tests for a Node's own local physical-load
 *        configuration (used identically by both Central and Smart Node).
 *
 ...
 * Load rather than leaving an orphaned channel behind. No Load configured
 * through this store ever has a per-load INA219 in the final hardware
 * design — only Central's own battery-bus INA219 reads real current.
 */
...
using kilowatts::NodeLoadHardwareStore;
...
NodeLoadHardwareStore::LoadConfiguration validConfiguration()
{

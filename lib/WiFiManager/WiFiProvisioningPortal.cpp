...
const char FORM_TAIL[] =
    ...
    "<h3>MQTT broker (required)</h3>"
    "<p>Enter the MQTT broker used by this installation. These values are "
    "stored on the Central Node and loaded after restart.</p>"
    "<label>Broker host or IP (required)</label><br>"
...
const char PAGE_MQTT_REJECTED[] =
    ...
    "Central is restarting, but MQTT will remain disabled until a valid broker is saved.</p></body></html>";
...
{
    /*
     * A match is only accepted at the start of the body or right after a
     * '&' field separator - an unanchored strstr() would let a short key
     * like "password=" match inside a longer one like "mqtt_password=",
     * silently reading the wrong field.
     */
    const std::size_t keyLength = std::strlen(key);
    const char* cursor = body;
    while (true) {
        cursor = std::strstr(cursor, key);
        if (cursor == nullptr) {
            return false;
        }
...
esp_err_t handlePostSave(httpd_req_t* req)
{
    ...
    /*
     * A broker host is required for MQTT. A blank host leaves any existing
     * broker configuration unchanged, so the installer can update Wi-Fi
     * without accidentally erasing MQTT settings.
     */
...
bool WiFiProvisioningPortal::begin(std::uint8_t channel)
{
    ...
    if (xTaskCreate(dnsResponderTask, "wifi_prov_dns", 3072U, nullptr, 3U, &dnsTaskHandle) != pdPASS) {
        ESP_LOGE(TAG, "wifi-provisioning: DNS responder task creation failed (task=wifi_prov_dns)");
    }

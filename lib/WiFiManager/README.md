...
- **`WiFiProvisioningPortal`** — the self-hosted "Kilowatts-Setup-XXXX"
  ...
  provisions an MQTT broker (host/port/TLS/username/password, saved via
  `MqttCredentialsStore`) — see `MqttManager`'s README. Does not
...
MQTT broker credentials are installer-provisioned through
`MqttCredentialsStore` and loaded from NVS; there is no compiled-in
broker fallback — see `MqttManager`'s own README.

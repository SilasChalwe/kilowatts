...
static const char* TAG = "NODE_LOAD_HW_STORE";
...
bool NodeLoadHardwareStore::applyOne(const LoadConfiguration& configuration,
...
void NodeLoadHardwareStore::rollbackOne(const LoadConfiguration& configuration,
                                              RelayController& relays,
                                              Node& node)
{
    node.removeLoadByRelayPin(configuration.relayPin);
...
bool NodeLoadHardwareStore::configureNewLoad(const LoadConfiguration& configuration,
                                                   RelayController& relays,
                                                   Node& node,
                                                   HardwareConfigurationFailureReason& failureReason)
{
...
bool NodeLoadHardwareStore::branchMaximumCurrentAmps(std::uint8_t relayPin, float& value) const
{
...
bool NodeLoadHardwareStore::loadPersisted()
{
    ...
    for (const PersistedLoadConfiguration& entry : persisted) {
        ...
        NodeLoadHardwareStore validationStore;
        ...
        if (!validationStore.isValidNewConfiguration(configuration, reason)) {
            ESP_LOGW(TAG, "Discarding invalid persisted Node load configuration (reason=%u)",
    ...
    ESP_LOGI(TAG, "Restored %u Node load configuration(s)",

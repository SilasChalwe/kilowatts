...
public:
    ...
    struct CommissioningRecord {
        ...
        /** commandId of the in-flight requestCommissioning() call; meaningless once syncState is not PENDING. Lets applyCommissionResult() reject a stale/duplicate ack for a superseded command. */
        std::uint32_t pendingCommandId;
     ...
     *
     * On acceptance, pendingFriendlyName and pendingCommandId are set and
     * syncState becomes PENDING; the caller must still send
     * CommissionCommandPacket (carrying the same commandId) over ESP-NOW
     * and later call applyCommissionResult() with the reply.
     */
    bool requestCommissioning(const MacAddress& macAddress, const char* friendlyName, std::uint32_t commandId);


    /**
     * Applies a Node's own CommissionAckPacket. Rejected (returns false,
     * record left untouched) when no record exists for macAddress, or
     * syncState is not PENDING, or commandId does not match
     * pendingCommandId - this is what lets a delayed/duplicate ack for a
     * superseded command (e.g. a decommission or a second rename issued
     * before the first one's ack arrived) be ignored instead of reverting
     * state a newer command already moved past.
     *
     * On a matching call, lifecycleState is always set to resultingState -
     * the Node's own report of its state is authoritative over anything
     * Central assumed. On success, friendlyName is committed from
     * pendingFriendlyName and syncState becomes SYNCED; otherwise it's
     * discarded and syncState becomes FAILED.
     */
    bool applyCommissionResult(const MacAddress& macAddress, std::uint32_t commandId,
                                bool success, NodeLifecycleState resultingState);

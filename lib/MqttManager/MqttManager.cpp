...
void formatMac(char* buffer, std::size_t size, const Load::MacAddress& mac)
{
...
bool parseSystemAction(const char* text, SystemCommandAction& action)
{
    ...
    if (std::strcmp(text, "APPLY_SAFETY_CONFIG") == 0) {
        action = SystemCommandAction::APPLY_SAFETY_CONFIG; return true;
    }
    return false;
}
...
{
    state_.store(MqttConnectionState::DISCONNECTED);
    ESP_LOGW(TAG, "Disconnected; local control continues");
}
...
void MqttManager::handleLoadCommandMessage(const char* data, std::size_t dataLength)
{
    ...
    char target[18]{};
    formatMac(target, sizeof(target), request.nodeMacAddress);
...
void MqttManager::handleSystemCommandMessage(const char* data, std::size_t dataLength)
{
    ...
    if (request.action == SystemCommandAction::APPLY_SAFETY_CONFIG) {
        const cJSON* policy = cJSON_GetObjectItemCaseSensitive(root, "safetyConfig");
        if (!cJSON_IsObject(policy) ||
            !readFloat(cJSON_GetObjectItemCaseSensitive(policy, "minimumStateOfChargePercent"), request.minimumStateOfChargePercent) ||
            !readFloat(cJSON_GetObjectItemCaseSensitive(policy, "warningStateOfChargePercent"), request.warningStateOfChargePercent) ||
            !readFloat(cJSON_GetObjectItemCaseSensitive(policy, "targetRuntimeHours"), request.targetRuntimeHours) ||
            !readFloat(cJSON_GetObjectItemCaseSensitive(policy, "safetyFactor"), request.safetyFactor) ||
            !readFloat(cJSON_GetObjectItemCaseSensitive(policy, "maximumBatteryDischargeCurrentAmps"), request.maximumBatteryDischargeCurrentAmps) ||
            !readFloat(cJSON_GetObjectItemCaseSensitive(policy, "maximumMainCurrentAmps"), request.maximumMainCurrentAmps)) {
            cJSON_Delete(root);
            publishAcknowledgement(request.commandId, "SYSTEM", AckStatus::REJECTED, "invalid safetyConfig", nullptr);
            return;
        }
...
void MqttManager::handleConfigCommandMessage(const char* data, std::size_t dataLength)
{
    ...
    if (request.action == ConfigCommandAction::CONFIGURE_LOAD) {
        ...
        if (!cJSON_IsObject(load) ||
            ...
            !readFloat(cJSON_GetObjectItemCaseSensitive(load, "nominalVoltageVolts"), request.nominalVoltageVolts) ||
            !readFloat(cJSON_GetObjectItemCaseSensitive(load, "nominalCurrentAmps"), request.nominalCurrentAmps) ||
            !readFloat(cJSON_GetObjectItemCaseSensitive(load, "startupWatts"), request.startupWatts) ||
        ...
        const float runningWatts = request.nominalVoltageVolts * request.nominalCurrentAmps;
        if (request.nominalVoltageVolts <= 0.0F || request.nominalCurrentAmps <= 0.0F ||
            !std::isfinite(runningWatts) || runningWatts <= 0.0F || request.startupWatts < runningWatts) {
            cJSON_Delete(root);

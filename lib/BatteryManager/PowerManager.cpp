 ...
 * @brief Central battery measurement and power-budget implementation.
 */
...
bool writeRegister(i2c_master_dev_handle_t device, std::uint8_t address, std::uint16_t value)
{
    const std::uint8_t bytes[3]{address,
        static_cast<std::uint8_t>((value >> 8U) & 0xFFU),
        static_cast<std::uint8_t>(value & 0xFFU)};
    return i2c_master_transmit(device, bytes, sizeof(bytes), I2C_TIMEOUT_MS) == ESP_OK;
}

bool readRegister(i2c_master_dev_handle_t device, std::uint8_t address, std::uint16_t& value)
{
    std::uint8_t bytes[2]{};
    if (i2c_master_transmit_receive(device, &address, 1U, bytes, sizeof(bytes), I2C_TIMEOUT_MS) != ESP_OK) {
        return false;
    }
    ...
    const BatteryConfiguration& batteryConfiguration)
{
    ...
    if (!simulationEnabled_) {
        if (!isFinitePositive(sensorConfiguration.shuntResistanceOhms) ||
            ...
            sensorConfiguration.emaAlpha <= 0.0F || sensorConfiguration.emaAlpha > 1.0F) {
            return false;
        }

        const float maximumExpectedShuntVoltage =
            sensorConfiguration.shuntResistanceOhms * sensorConfiguration.maximumExpectedCurrentAmps;
        if (maximumExpectedShuntVoltage > MAXIMUM_SHUNT_VOLTAGE_VOLTS) return false;
    }
...
bool PowerManager::isInitialized() const { return initialized_; }
...
bool PowerManager::initializeSensor()
{
    ...
    if (deviceHandle_ == nullptr) {
        ...
        if (i2c_master_bus_add_device(
                reinterpret_cast<i2c_master_bus_handle_t>(busHandle_), &config, &device) != ESP_OK) {
            return false;
        }
...
bool PowerManager::isHardwareSensorPresent() const
{
    ...
    return busHandle_ != nullptr &&
           i2c_master_probe(reinterpret_cast<i2c_master_bus_handle_t>(busHandle_),
                            INA219_I2C_ADDRESS, I2C_TIMEOUT_MS) == ESP_OK;
#else
    return false;
#endif
}

bool PowerManager::readHardwareMeasurements(PowerMeasurements& measurements) const
{
#ifdef ESP_PLATFORM
    ...
    const float voltage = static_cast<float>(busRaw >> 3U) * BUS_VOLTAGE_LSB_VOLTS;
    const float shuntVoltage = static_cast<float>(static_cast<std::int16_t>(shuntRaw)) * SHUNT_VOLTAGE_LSB_VOLTS;
    const float current = shuntVoltage / sensorConfiguration_.shuntResistanceOhms;
    const float power = voltage * current;
    if (!std::isfinite(voltage) || !std::isfinite(current) || !std::isfinite(power)) return false;
...
bool PowerManager::isSimulationEnabled() const { return simulationEnabled_; }

bool PowerManager::setSimulatedMeasurements(float voltage, float current)
{
    if (!std::isfinite(voltage) || voltage <= 0.0F || !std::isfinite(current)) return false;
...
PowerManager::Calibration PowerManager::getCalibration() const { return calibration_; }
    ...
    const Calibration& calibration)
{
    ...
    result.currentAmps = (measurements.currentAmps + calibration.currentOffsetAmps) * calibration.currentScaleFactor;
...
bool PowerManager::updateMeasurements()
{
    ...
    if (!std::isfinite(calibrated.voltageVolts) ||
        ...
        !std::isfinite(calibrated.powerWatts)) return false;

    filteredMeasurements_ = hasFilteredMeasurement_
        ? applyExponentialMovingAverage(filteredMeasurements_, calibrated, sensorConfiguration_.emaAlpha)
        : calibrated;
...
PowerMeasurements PowerManager::getMeasurements() const { return measurements_; }
MeasurementSource PowerManager::getMeasurementSource() const { return measurementSource_; }
...
bool PowerManager::initializeStateOfCharge(float starting, bool restorePersistedState)
{
...
{
    if (!stateOfChargeInitialized_ || !isFinitePositive(deltaTimeSeconds) ||
        !std::isfinite(measurements_.currentAmps) ||
        !isFinitePositive(batteryConfiguration_.capacityAmpHours)) return false;

    const float change = measurements_.currentAmps * deltaTimeSeconds * 100.0F /
...
bool PowerManager::applyVoltageStateOfChargeEstimate(float emptyV, float fullV, float weight)
{
    if (!stateOfChargeInitialized_ || !isFinitePositive(emptyV) || !isFinitePositive(fullV) ||
        fullV <= emptyV || !std::isfinite(weight) || weight < 0.0F || weight > 1.0F ||
        !std::isfinite(measurements_.voltageVolts)) return false;
...
float PowerManager::getStateOfChargePercent() const { return stateOfChargePercent_; }
StateOfChargeSource PowerManager::getStateOfChargeSource() const { return stateOfChargeSource_; }
...
bool PowerManager::persistStateOfChargeValue(float stateOfChargePercent) const
{
    ...
    const bool ok = writeFloat(handle, SOC_NVS_KEY, stateOfChargePercent) && nvs_commit(handle) == ESP_OK;
    nvs_close(handle);
    return ok;
#else
    (void)stateOfChargePercent;
...
{
    return isStateOfChargeValid() && persistStateOfChargeValue(stateOfChargePercent_);
}
...
float PowerManager::getRemainingRequiredRuntimeHours() const { return remainingRequiredRuntimeHours_; }
...
bool PowerManager::updatePowerBudget()
{
    ...
    if (!isFinitePositive(P_budget) || !isFiniteNonNegative(P_reserve) ||
        !isFiniteNonNegative(P_fixed) || !isFiniteNonNegative(P_auto) ||
        P_reserve > P_budget) {
        return false;
    }
    ...
    if (runtimeBudgetActive) {
        ...
        const float usableSocFraction = std::max(
            0.0F,
            (stateOfChargePercent_ - batteryConfiguration_.minimumStateOfChargePercent) / 100.0F);
        const float usableEnergyWh = batteryConfiguration_.capacityAmpHours *
    ...
    if (isStateOfChargeValid() &&
        stateOfChargePercent_ <= batteryConfiguration_.minimumStateOfChargePercent) {
        planningAllowance = 0.0F;
        if (runtimeBudgetActive && P_fixed > 0.0F) requiredRuntimeAchievable = false;
    }

    powerBudget_.P_usable = P_usable;
    powerBudget_.P_auto_available = std::max(0.0F, planningAllowance - P_fixed);
    powerBudget_.P_remaining = std::max(0.0F, P_budget - (P_fixed + P_auto));
...
PowerBudget PowerManager::getPowerBudget() const { return powerBudget_; }
float PowerManager::getAutoAvailablePowerWatts() const { return powerBudget_.P_auto_available; }
float PowerManager::getRemainingPowerWatts() const { return powerBudget_.P_remaining; }

void PowerManager::printDiagnosticReport() const
{
#ifdef ESP_PLATFORM
    ESP_LOGI(TAG, "Measurement source: %s", toText(measurementSource_));
    ESP_LOGI(TAG, "Measured: %.2f V | %.3f A | %.2f W",
    ...
    ESP_LOGI(TAG, "P_budget=%.2f P_reserve=%.2f P_usable=%.2f",
    ...
    ESP_LOGI(TAG, "P_fixed=%.2f P_auto_available=%.2f P_auto=%.2f P_remaining=%.2f",
    ...
    if (powerBudget_.runtimeBudgetActive) {
        ESP_LOGI(TAG, "Runtime remaining=%.2f h P_runtime=%.2f W achievable=%s",
